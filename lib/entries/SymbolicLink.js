  ...
  if (typeof callback === 'function') {
    ...
    try {
      ...
      if (!isAbsolute(self.linkpath)) {
        var targetRelativePath = path.join(path.dirname(self.path), self.linkpath);
        var targetFullPath = path.join(dest, stripPath(targetRelativePath, options));
        linkpath = path.relative(path.dirname(fullPath), targetFullPath);
      }
      ...
      queue.defer(symlink.bind(fs, linkpath, fullPath));
