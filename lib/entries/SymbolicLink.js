...
SymbolicLinkEntry.prototype.create = function create(dest, options, callback) {
  ...
  if (typeof callback === 'function') {
    ...
    try {
      ...
      var linkFullPath = self.linkpath;
      var linkpath = self.linkpath;
      if (!isAbsolute(self.linkpath)) {
        var linkRelativePath = path.join(path.dirname(self.path), self.linkpath);
        linkFullPath = path.join(dest, stripPath(linkRelativePath, options));
        linkpath = path.relative(path.dirname(fullPath), linkFullPath);
      }
      ...
      if (isWindows) queue.defer(symlinkWin32.bind(null, linkFullPath, linkpath, fullPath));
      else queue.defer(fs.symlink.bind(fs, linkpath, fullPath));
